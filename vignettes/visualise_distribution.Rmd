...
student %>% 
  dplyr::filter(country %in% selected_countries, year == 2003)
...
country_lm = country_summ %>% 
  ...
  nest() %>% 
  dplyr::mutate(
    lm = purrr::map(.x = data, ~ lm(math_p50 ~ year, data = .x)),
    lm_tidy = purrr::map(lm, broom::tidy)) %>% 
  dplyr::select(country, lm_tidy) %>% 
  tidyr::unnest(lm_tidy) %>% 
  dplyr::select(country, term, estimate, p.value) %>% 
  dplyr::mutate(term = ifelse(term == "(Intercept)", "int", term)) %>% 
...
```{r}
student %>% 
  dplyr::filter(country %in% c("AUS", "USA", "JPN", "BRA")) %>% 
...
obj = lmer(math ~ escs + (escs|year/school_id) + (escs|gender), data = aus_data)
summary(obj)
```

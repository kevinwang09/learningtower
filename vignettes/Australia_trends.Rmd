---
title: "Oi! How's 'Straya doing in the PISA study (someone can think of a better name)"
...
Since we are expecting some time variations in the data, it is a good idea to quickly visualise the time trends. 
...
In order to reduce the complexity in modelling, we will first explore the most recent 2018 data before generalising the models/results into any trends. 

# Linear regression model for the 2018 study

We will first perform a preliminary exploration of the data using linear models. We will first fit three linear models (one for each subject of maths, reading and science) on the 2018 Australian data to get a sense of the main variables that may be influencing test scores.

We will subset the `student` data to only AUS and relevel some factors for analysese later. 
```{r}
...
1. All three response variables seem to be influenced by similar factors.

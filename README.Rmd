...
The `learningtower` gives access to a subset of variables from PISA data originally collected and are available from [OECD](https://www.oecd.org/en/about/programmes/pisa/pisa-data.html), collected on a three year basis.
...
Note that because of changing data specification over the survey years, not all variables were measured consistently across all cycles (2000–2022). The summary figure below illustrates the missingness patterns and variable presence across survey years for the student dataset:
...
See `?school` for more information on the different variables present in the school dataset. The summary figure below illustrates the missingness patterns and variable presence across survey years for the school dataset:
